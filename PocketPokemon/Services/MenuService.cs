...
public class MenuService {
    ...
    GetHp:
        ...
        if (!int.TryParse(hp, out int intHp)) {
            Console.WriteLine("Hp must be a valid integer.\nTry again, or enter \"cancel\" to cancel.\n");
            goto GetHp;
        }
    ...
    private void RemovePokemon() {
        ...
        ConfirmRemove:
            char confirmRemove = GetInput($"\n!!! This process cannot be undone !!!\nConfirm removal of {toRemove.Name}?")!;

            switch (char.ToLower(confirmRemove)) {
            case 'y':
                Console.WriteLine();
                if (_Service.RemovePokemon(toRemove)) {
                    removed++;
                    Console.WriteLine($"{toRemove.Name} removed successfully!");
                } else {
                    Console.WriteLine($"Could not remove {toRemove.Name}\nPlease try again!!");
                    continue;
                }
                break;
            case 'n':
